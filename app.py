...
def get_coordinates(city_name):
    """Convert any city name to lat/lon using OpenWeatherMap Geocoding API"""
...
def get_professional_weather_report(query):
    """Main AI: Understand query and generate military/pilot grade report"""
    query_lower = query.lower()

    # 1. Find Location from query
    ...
    for kw in location_keywords:
        if kw in query_lower:
            ...
            if len(parts) > 1:
                location = parts[1].strip().split('?')[0].split('.')[0]
                break

    lat, lon, full_location_name = get_coordinates(location)
    if not lat:
        return f"❌ **Location Error**: {full_location_name}. Please specify a valid city/country. Ex: 'Weather in Pokhara' or 'Mausam in Mt Everest'"

    # 2. Get Weather + Air Quality Data
    try:
        ...
        return "❌ **API Error**: Weather service down or API Key issue. Check Streamlit Secrets."

    # 3. Build Professional Report based on Query Intent
    temp = w['main']['temp']
    ...
    wind_deg = w['wind']['deg']
    clouds = w['clouds']['all']
    visibility = w.get('visibility', 10000) / 1000 # km
    ...
    # Disaster/Military Report
    report += f"#### ⚠️ **Disaster & Safety Alert**\n"
    report += f"- **Air Quality Index**: {aqi_text} ({aqi}/5)\n"
    if "tsunami" in query_lower:
        report += f"- **Tsunami Risk**: No active tsunami warnings for this inland/coastal area. Check official INCOIS/PTWC for coastal alerts.\n"
    ...
    if "snow" in query_lower or "himpat" in query_lower:
        report += f"- **Snow/Blizzard Risk**: {'High' if temp < 0 and clouds > 80 else 'Low'}.\n"
    if "everest" in query_lower:
        report += f"- **Mt Everest Base Camp Risk**: Wind Chill {feels_like-10:.1f}°C. Oxygen level low. AQI: {aqi_text}.\n"

    report += f"\n*Data Source: OpenWeatherMap & NASA. For critical military/pilot use, verify with official METAR/TAF.*"
    return report

def get_nasa_intel(query):
    """NASA Data for Space/Military Queries"""
    query_lower = query.lower()
    if "iss" in query_lower:
        iss = requests.get("http://api.open-notify.org/iss-now.json", timeout=10).json()
        return f"**ISS Live Intel**: Currently over {iss['iss_position']['latitude']}, {iss['iss_position']['longitude']}. Orbit speed 28,000 km/h. Next pass over Nepal in ~90 mins."
    ...
    if "mars" in query_lower:
        return "**NASA Mars Intel**: Curiosity Rover active. Red planet 225M km away. Use NASA Space tab for latest photos."
    return None

# --- UI: PURE CHAT INTERFACE LIKE CHATGPT ---
st.title("🛰️ SkyGPT Command Center v2.0")
...
st.divider()
st.caption("v2.0 Command Center | © 2026 SkyGPT | Data: NASA.gov, OpenWeatherMap.org | Not for official navigation. Verify with authorized sources.")
